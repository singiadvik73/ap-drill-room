...
Free AP practice for **AP Human Geography**, **AP Biology**, **AP World History: Modern**, **AP Chemistry**, **AP Precalculus**, and **AP Psychology**: a question pool for every CED unit and topic, AP-style figure questions, progress tracking, accounts, and an admin Dev mode for editing questions. Every HuG and Bio topic has at least one figure question.
...
- **From the Claude version:** questions made there reach the site when the person uses **Link Google account** and signs in. The relay adds them to the shared bank right away, labeled **Made in the Claude version**. In the Claude version, every new batch goes through a second Claude request that checks accuracy, one correct answer, topic fit and appropriateness. Failing questions are removed, and only questions that passed are shared. Both requests use the student's own Claude usage, and the consent screen spells that out. That check runs in the student's browser, so someone who tampers with the page could skip it. The relay itself only checks the format (known topic, one correct answer and three different wrong ones), skips duplicates (`submissions/<hash>`), and caps each account at 40 a day (`quota/`). Hide bad ones in Dev mode.
